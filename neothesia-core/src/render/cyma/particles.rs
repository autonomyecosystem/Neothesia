...
impl ParticleRenderer {
    ...
    pub(super) fn update(
        ...
        for index in 0..count {
            ...
            let gradient = field.node_gradient(state.position[0], state.position[1], 0.0035);
            state.velocity[0] =
                (state.velocity[0] - gradient[0] * attraction * delta_seconds) * damping;
            state.velocity[1] =
                (state.velocity[1] - gradient[1] * attraction * delta_seconds) * damping;
            ...
            let distance = field.sample(state.position[0], state.position[1]).abs();
            self.instances[index] = ParticleInstance {
                position: state.position,
                opacity: field.activity * (1.0 / (1.0 + distance * 22.0)),
...
mod tests {
    ...
    fn particle_reset_is_deterministic() {
        ...
        for (index, state) in first.iter_mut().enumerate() {
            let seed = index as f32 + 1.0;
            let angle = fract(seed * 0.618_034) * std::f32::consts::TAU;
            *state = ParticleState {
                position: [fract(seed * 0.754_877_7), fract(seed * 0.569_840_3)],
    ...
    fn field_gradient_used_by_particles_is_finite() {
        ...
        for index in 0..32 {
            ...
            let gradient = field.node_gradient(position[0], position[1], 0.0035);
